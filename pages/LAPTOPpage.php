...
try {
    ...
    if ($searchTerm) {
        $searchCondition = "WHERE (
            CONCAT('LAP-', LPAD(la.asset_id, 5, '0')) LIKE :search
            OR la.serial_num LIKE :search
            OR la.brand LIKE :search
            OR la.model LIKE :search
            OR sl.staff_name LIKE :search
        )";
        $searchParams[':search'] = '%' . $searchTerm . '%';
    }
    
    $countQuery = "SELECT COUNT(*) FROM laptop_desktop_assets la LEFT JOIN staff_list sl ON la.staff_id = sl.staff_id $searchCondition";
        ...
        <div class="search-section">
            <form method="GET" action="" id="searchForm">
                <div class="search-box">
                    ...
                    <input type="text" name="search" placeholder="Search Asset ID, Serial, Brand, Model, Assigned To..." id="searchInput" value="<?php echo htmlspecialchars($searchTerm); ?>">
                </div>
            </form>
        </div>
