...
try {
    ...
    if ($searchTerm) {
        $searchCondition = "WHERE (
            CONCAT('AV-', LPAD(aa.asset_id, 5, '0')) LIKE :search
            OR aa.serial_num LIKE :search
            OR aa.brand LIKE :search
            OR aa.model LIKE :search
            OR aa.location LIKE :search
        )";
        $searchParams[':search'] = '%' . $searchTerm . '%';
    }
    
    $countQuery = "SELECT COUNT(*) FROM av_assets aa $searchCondition";
        ...
        <div class="search-section">
            <form method="GET" action="" id="searchForm">
                <div class="search-box">
                    ...
                    <input type="text" name="search" placeholder="Search Asset ID, Serial, Brand, Model, Location..." id="searchInput" value="<?php echo htmlspecialchars($searchTerm); ?>">
                </div>
            </form>
        </div>
