...
class ReturnPDFGenerator {
    ...
    private function addReturnPage($data) {
        ...
        $colWidths = [($pageWidth * 0.35), ($pageWidth * 0.25), ($pageWidth * 0.40)];
        ...
        $pageWidth = $this->pdf->GetPageWidth() - 30;
        $tableColWidths = [($pageWidth * 0.30), ($pageWidth * 0.35), ($pageWidth * 0.35)];
        ...
        foreach ($subItems as $item => $details) {
            $indent = 10;
            $this->pdf->Cell($indent, 7, '', 0, 0, 'L');
            $this->pdf->Cell($tableColWidths[0] - $indent, 7, $item, 1, 0, 'L', true);
        ...
        $pageWidth = $this->pdf->GetPageWidth() - 30;
        $gap = 10;
        $col1Width = ($pageWidth - $gap) / 2;
        $col2Width = ($pageWidth - $gap) / 2;
        ...
        // Checked by section
        $this->pdf->SetX($startX + $col1Width + $gap);
        ...
        // Handover by details
        $this->pdf->Cell($labelWidth, 6, 'Name:', 0, 0, 'L');
        $this->pdf->Cell($col1Width - $labelWidth, 6, $data['staff_name'] ?? '', 0, 0, 'L');
        
        // Checked by details
        $this->pdf->SetX($startX + $col1Width + $gap);
        $this->pdf->Cell($labelWidth, 6, 'Name:', 0, 0, 'L');
        $this->pdf->Cell($col2Width - $labelWidth, 6, $data['received_by_name'] ?? 'IT Department', 0, 1, 'L');
        
        $this->pdf->Cell($labelWidth, 6, 'Designation:', 0, 0, 'L');
        $this->pdf->Cell($col1Width - $labelWidth, 6, $data['staff_department'] ?? '', 0, 0, 'L');
        
        $this->pdf->SetX($startX + $col1Width + $gap);
        $this->pdf->Cell($labelWidth, 6, 'Designation:', 0, 0, 'L');
        $this->pdf->Cell($col2Width - $labelWidth, 6, $data['received_by_designation'] ?? 'IT TECHNICIAN', 0, 1, 'L');
        
        $this->pdf->Cell($labelWidth, 6, 'Date:', 0, 0, 'L');
        $this->pdf->Cell($col1Width - $labelWidth, 6, $data['return_date'] ?? date('d.m.Y'), 0, 0, 'L');
        
        $this->pdf->SetX($startX + $col1Width + $gap);
        $this->pdf->Cell($labelWidth, 6, 'Date:', 0, 0, 'L');
        $this->pdf->Cell($col2Width - $labelWidth, 6, $data['return_date'] ?? date('d.m.Y'), 0, 1, 'L');
